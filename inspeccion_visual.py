...
def get_canal_data():
    ...
    df = pd.read_csv('canal_volumen.csv') 
...
plt.plot(df_data['fecha'], df_data['volumen'], 
...
df_data['tendencia_7d'] = df_data['volumen'].rolling(window=7).mean()
plt.plot(df_data['fecha'], df_data['tendencia_7d'], 
         color='#004b8d', linewidth=2, label='Promedio Móvil (7 días)')

plt.title('Inspección Gráfica: Demanda Transaccional - Canal', fontsize=14)
